...
import { GitStateStore } from '../src/state-store';
...
function rejectingRemote(): string {
  const bare = mkdtempSync(join(tmpdir(), 'b4-notifier-remote-'));
  git(bare, ['init', '--bare', '.']);
...
function workRepo(): string {
  const work = mkdtempSync(join(tmpdir(), 'b4-notifier-work-'));
  git(work, ['init', '.']);
...
describe('B4: GitStateStore surfaces a redacted push failure', () => {
  it('returns pushError (not a swallowed catch) when the remote rejects the push', async () => {
    const store = new GitStateStore(workRepo(), { remote: rejectingRemote() });
    const result = await store.save('{"schema_version":"1.0"}\n', 'test commit');
